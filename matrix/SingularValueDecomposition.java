 ...
 * (https://github.com/apache/commons-math/blob/master/src/main/java/org/apache/commons/math4/linear/SingularValueDecomposition.java)
 *
 * Modified by Lucien Gaitskell for Wheeler Robotics (2018-19)
 *
 * Changed:
 *  1) Implements FTC matrix objects
 *  2) Removed #getCovariance(), as it depends on many Apache libraries + add unneeded functionality
 *
 *  NOTE: FTC matrices are floats ONLY
 *      (double precision remains, due to issues in the results that arose from removing it.
 *       Likely due to premature rounding)
 */
package org.wheelerschool.robotics.robotlib.matrix;
...
public class SingularValueDecomposition {
    ...
    private final MatrixF cachedU;
    /** Cached value of transposed U matrix. */
    private MatrixF cachedUt;
    /** Cached value of S (diagonal) matrix. */
    private MatrixF cachedS;
    /** Cached value of V matrix. */
    private final MatrixF cachedV;
    /** Cached value of transposed V matrix. */
    private MatrixF cachedVt;
    /**
    ...
    private static double[][] getMatData(MatrixF matrixF) {
        double[][] result = new double[matrixF.numRows()][matrixF.numCols()];

        for (int r=0; r<matrixF.numRows(); r++) {
            for (int c=0; c<matrixF.numCols(); c++) {
                result[r][c] = matrixF.get(r, c);
            }
        }
        return result;
    }

    private static float[] flattenConv(double[][] arr) {
        float[] result = new float[arr.length * arr[0].length];
        for (int r=0; r<arr.length; r++) {
            for (int c=0; c<arr[r].length; c++) {
                result[arr[r].length*r + c] = (float) arr[r][c];
            }
        }
        return result;
    }
     ...
     */
    public SingularValueDecomposition(final MatrixF matrix) {
        final double[][] A;

        // "m" is always the largest dimension.
        if (matrix.numRows() < matrix.numCols()) {
            transposed = true;
            A = getMatData(matrix.transposed());
            m = matrix.numCols();
            n = matrix.numRows();
        } else {
            transposed = false;
            ...
            switch (kase) {
                ...
                case 3: {
                    ...
                    for (int j = k; j < p - 1; j++) {
                        ...
                        singularValues[j + 1] = (float) cs * singularValues[j + 1];
                        ...
                        singularValues[j + 1] = (float) (-sn * e[j] + cs * singularValues[j + 1]);
                        g = sn * e[j + 1];
                        e[j + 1] = cs * e[j + 1];
                        if (j < m - 1) {
                            for (int i = 0; i < m; i++) {
        ...
        if (!transposed) {
            cachedU = new GeneralMatrixF(U.length, U[0].length, flattenConv(U));  //MatrixUtils.createRealMatrix(U);
            cachedV = new GeneralMatrixF(V.length, V[0].length, flattenConv(V));  //MatrixUtils.createRealMatrix(V);
        } else {
            cachedU = new GeneralMatrixF(V.length, V[0].length, flattenConv(V));  //MatrixUtils.createRealMatrix(V);
            cachedV = new GeneralMatrixF(U.length, U[0].length, flattenConv(U));  //MatrixUtils.createRealMatrix(U);
        }
    }
     ...
     * @see #getU()
     */
    public MatrixF getUT() {
        if (cachedUt == null) {
            cachedUt = getU().transposed();
        }
        // return the cached matrix
        return cachedUt;
    }
    ...
    public MatrixF getS() {
        if (cachedS == null) {
            ...
            cachedS = MatrixF.diagonalMatrix(new VectorF(toFloat(singularValues)));
        }
        return cachedS;
    }
    ...
    public static class Solver {  // implements DecompositionSolver
        ...
        private final MatrixF pseudoInverse;
         ...
         */
        private Solver(final double[] singularValues, final MatrixF uT,
                       final MatrixF v, final boolean nonSingular, final double tol) {
            final double[][] suT = getMatData(uT);
            for (int i = 0; i < singularValues.length; ++i) {
                final double a;
                if (singularValues[i] > tol) {
            ...
            pseudoInverse = v.multiplied(new GeneralMatrixF(suT.length, suT[0].length, flattenConv(suT)));
         ...
         */
        public VectorF solve(final VectorF b) {
            return pseudoInverse.multiplied(b);
        }
         ...
         * if the matrices dimensions do not match.
         */
        public MatrixF solve(final MatrixF b) {
            return pseudoInverse.multiplied(b);
        }
